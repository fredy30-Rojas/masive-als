...
7. El filtro CNS/BBB ya NO descarta: calcula un score CNS-MPO aproximado
   (0-6) para priorizar sin eliminar candidatos por un corte binario.
...
def unir_smiles_desde_libreria(df: pd.DataFrame, cols: dict, ruta_lib: str) -> pd.DataFrame:
    ...
    print(f"[Union SMILES] {antes} filas de resultados unidas contra '{ruta_lib}' "
          ...
          f"{sin_smiles} filas quedaron SIN smiles tras la union.")
    cols["smiles"] = "smiles"
    return df


def deduplicar_por_smiles_canonico(df: pd.DataFrame, cols: dict) -> pd.DataFrame:
    """Deduplica por SMILES canonico DENTRO de cada proteina, quedandose con la mejor afinidad."""
    try:
    ...
    con_smiles = con_smiles.sort_values(cols["afinidad"])
    deduplicado = con_smiles.drop_duplicates(subset=[cols["proteina"], "_smiles_canonico"], keep="first")

    print(f"[Dedup] {invalidos} filas con SMILES ausente/invalido. De las {antes} validas: "
          f"{len(deduplicado)} tras deduplicar (se descartaron {antes - len(deduplicado)} duplicados).")
...
def calcular_enrichment(df_top_crudo: pd.DataFrame, cols: dict, ruta_controles: str, top_pct: float) -> None:
    """Calcula enrichment SOBRE LA SELECCION CRUDA, ANTES de PAINS/Lipinski/CNS."""
    ...
    if id_ctrl is None or prot_ctrl is None:
        print(f"ERROR: el CSV de controles no tiene columnas reconocibles. "
              f"Columnas: {list(controles.columns)}")
        return

    ids_top = set(zip(df_top_crudo[cols["proteina"]], df_top_crudo[cols["compuesto"]]))
    print("\n[Validacion] Enrichment de controles positivos (ANTES de filtros):")
    for proteina, grupo in controles.groupby(prot_ctrl):
        ...
        print(f"  {proteina}: {encontrados}/{total} controles conocidos "
              f"aparecen en el top-{top_pct}% crudo por proteina")
    print("  (si esto da 0 para alguna proteina, revisar caja/bolsillo antes de confiar en su ranking)\n")
...
def paso2_drug_likeness_pains(df: pd.DataFrame, cols: dict) -> pd.DataFrame:
    ...
    filas_validas = []
    sin_smiles = rechazadas_pains = rechazadas_lipinski = 0
    ...
    print(f"[Paso 2] {len(df)} -> {len(resultado)} compuestos. "
          f"Descartadas: {sin_smiles} sin SMILES, {rechazadas_pains} por PAINS, "
          f"{rechazadas_lipinski} por Lipinski/Veber.")
    return resultado


def paso2b_score_cns_mpo(df: pd.DataFrame, cols: dict) -> pd.DataFrame:
    """Calcula un score CNS-MPO aproximado (0-6) para priorizar, SIN descartar a nadie."""
    from rdkit import Chem
    from rdkit.Chem import Descriptors

    def score_deseabilidad(valor, bajo, alto, invertir=False):
        if invertir:
            if valor <= bajo:
                return 1.0
            if valor >= alto:
                return 0.0
            return (alto - valor) / (alto - bajo)
        else:
            if valor <= bajo:
                return 0.0
            if valor >= alto:
    ...
    for _, fila in df.iterrows():
        ...
        d_mw = score_deseabilidad(mw, 360, 500, invertir=True)
        d_tpsa = score_deseabilidad(tpsa, 40, 90, invertir=True)
        d_logp = 1.0 if 1 <= logp <= 3 else max(0.0, 1 - abs(logp - 2) / 3)
        d_hbd = score_deseabilidad(hbd, 0, 3, invertir=True)

        score_cns_mpo_aprox = round((d_mw + d_tpsa + d_logp + d_hbd) / 4 * 6, 2)

        nueva_fila = fila.copy()
        nueva_fila["cns_mpo_aprox"] = score_cns_mpo_aprox
        nueva_fila["mw"] = round(mw, 1)
        nueva_fila["tpsa"] = round(tpsa, 1)
        filas_con_score.append(nueva_fila)

    resultado = pd.DataFrame(filas_con_score)
    if not resultado.empty:
        resultado = resultado.sort_values("cns_mpo_aprox", ascending=False)

    print(f"[Paso 2b - CNS-MPO] {len(df)} -> {len(resultado)} compuestos con score "
          f"(0-6, mayor = mejor). {sin_smiles} sin SMILES valido.")
    print("  NOTA: nadie fue eliminado por este paso. Revisa el CSV final ordenado "
          "por 'cns_mpo_aprox' antes de decidir cuales llevar a MM-GBSA.")
    return resultado
...
def paso3_preparar_para_mmgbsa(df: pd.DataFrame, cols: dict, ruta_salida: str) -> None:
    ...
    if df.empty:
        print("[Paso 3] AVISO: 0 compuestos sobrevivieron el embudo. No se exporta CSV.")
        return

    columnas_salida = [c for c in
                        [cols["compuesto"], cols["proteina"], cols["afinidad"],
                         cols["smiles"], cols["pose"], "cns_mpo_aprox", "mw", "tpsa"]
                        if c and c in df.columns]
    if not cols["pose"]:
        print("[Aviso] No hay columna de ruta a la pose acoplada: el resultado NO "
              "estara listo para MM-GBSA directamente, solo para revision.")
...
def main():
    ...
    parser.add_argument("--lib-smiles", default=None,
                         help="CSV de libreria con id de compuesto + smiles")
    ...
    parser.add_argument("--controles", default=None,
                         help="CSV opcional con ligandos conocidos para enrichment")
    ...
    if not cols["smiles"]:
        if not args.lib_smiles:
            print("ERROR: falta columna smiles y no se paso --lib-smiles.")
            sys.exit(1)
    ...
    if args.controles:
        calcular_enrichment(df_top_crudo, cols, args.controles, args.top_pct)

    df_filtrado = paso2_drug_likeness_pains(df_top_crudo, cols)
    df_filtrado = paso2b_score_cns_mpo(df_filtrado, cols)
